...
def parseOptions():
    ...
    if ('CMSSW_BASE' not in os.environ and opt.ANALYSISTYPE == 'cmsRun'):
...
def submitProduction():
    ...
    for job in range(1,int(nJOBS)+1):
        ...
        if(opt.ANALYSISTYPE == "script" or opt.ANALYSISTYPE == "scriptAndJOBID"):
            ...
            outScript.write('cd %s \n' %(subDir));
            outScript.write('export %s \n' %(SCRAM_ARCH));
            outScript.write('source ../../../settings.sh \n');    
            outScript.write('export X509_USER_PROXY=~/myVoms/x509up_u1282033 \n');
            outScript.write('source  %s/%s \n' %(subDir, cfgfile) )
            outScript.write( '\n ' );
            os.system('chmod 777 %s/%s' %(subDir, shFile));
    ...
    write_condorjob = open(currentDir+'/lanciaCondor_'+opt.TAGNAME+'.sub', 'w')
    write_condorjob.write('Universe = vanilla \n')
    write_condorjob.write('Executable  = '+outDir+"/JOB_$(JobID)"+'/bjob_$(JobID).sh \n')
    ...
    write_condorjob.write('output      = '+outDir+"/JOB_$(JobID)"+'/output.out \n')
    write_condorjob.write('error       = '+outDir+"/JOB_$(JobID)"+'/error.err \n')
    write_condorjob.write('log         = '+outDir+"/JOB_$(JobID)"+'/log_htc.log \n\n')
    write_condorjob.write('request_cpus = 4 \n')
    write_condorjob.write('max_retries = 1\n')
    write_condorjob.write('queue JobID in (')
    for job in range(1,int(nJOBS)):
        write_condorjob.write( str(job)+', ')
    write_condorjob.write( str(nJOBS)+') \n')
